...
import { fetchMemberMedications } from "@/entities/medication";
import { useCreatePastRecord } from "../api/useCreatePastRecord";
...
export const AddPastRecordForm: React.FC<AddPastRecordFormProps> = ({
  ...
  const createRecord = useCreatePastRecord();
  ...
  useEffect(() => {
    ...
    fetchMemberMedications(selectedMemberId)
      .then((meds) => {
        setMedications(meds.map((m) => ({ id: m.id, name: m.name, memberId: m.memberId })));
        setSelectedMedicationIds([]);
      })
      .catch(() => {
        setMedications([]);
