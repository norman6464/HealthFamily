...
import { DEFAULT_NOTIFICATION_SETTING } from "@/entities/notification-setting";
import {
...
export function NotificationSettingsForm({ setting, isLoading }: NotificationSettingsFormProps) {
  const updateMutation = useUpdateNotificationSetting();
  const onSave = (input: UpdateNotificationSettingInput) => updateMutation.mutateAsync(input);
