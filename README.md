...
**URL: https://d2i8wsadwlzu66.cloudfront.net**
...
- **クリーンアーキテクチャ適用** 🏗️
  ...
  - Data層: リポジトリ実装、APIクライアント（モック）
  - Presentation層: カスタムフック（ViewModel）による状態管理
  - 依存関係逆転の原則（DIP）に従った設計
  - テストカバレッジ向上（14個のテスト）
...
![AWSアーキテクチャ図](architecture/AWSアーキテクチャー図.png)
