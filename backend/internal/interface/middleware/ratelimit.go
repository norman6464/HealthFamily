...
// 「設定値 × インスタンス数」になる。Cloud Run の maxScale は 20 なので、
// 5回/分と書いてあっても全体では最大 100回/分まで通りうる。
//
// したがってコードの総当たりをこれ単独では止められない。実際の歯止めは
// アカウント側の試行回数上限 (usecase.MaxCodeAttempts) が持っており、
// ここは「1インスタンスあたりの粗い蛇口」として扱うこと。
// 厳密な全体制限が要るなら共有ストア(Memorystore 等)へ移す。
